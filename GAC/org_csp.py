...
class CSP:
    ...
    def is_consistent(self, var: int, value: dict, assignment: list) -> bool:
        ...
        var = {"attack" & "win_rate"} for the index team
        ...
        for unit in value["attack"]:
            for _, counter in assignment:
                # Same Unit exists in other assigned team
                if unit in counter["best_team"]["attack"]:
                    return False
...
def calculate(data: dict):
    ...
    domain = [
        (i, var)
        for i, var in enumerate([team for zone in data for team in zone["TEAMS"]])
    ]

    csp = CSP(variables=variables, domains=domain)
    sol = csp.solve()
    """
    Solution should return this format:
        {
            "T1": [
                {
                    "defense": [

...
def transform_team(team: list, all_existing_characters: list) -> tuple[list, bool]:
    ...
    for character_name in team:
        character_data = {}
        # Iterate through all existing characters to find the character ID
        for character_dict in all_existing_characters:
            if character_name == character_dict["name"]:
                character_data["name"] = character_dict["name"]
                character_data["base_id"] = character_dict["base_id"]
                character_data["categories"] = character_dict["categories"]
                character_data["image"] = character_dict["image"]
        ...
        # Otherwise append the character ID to the transformed team
            ...
            transformed_team.append(character_data)
...
def transform_data(
    ...
    """
        ...
        :param player_characters: List with all available player units
    ...
    # Get all existing characters in the game from API
    all_existing_characters = get_all_characters()

    # Divide the input data into opponent and player data
    oppone_data = data["opponent"]  # Opponenets defense
    player_data = data["player"]  # Players defense
    ...
    print(players_defense_units)
    # Remove those characters from player_characters
    available_players_characters = [
        character
        for character in player_characters
        if character not in players_defense_units
    ]
